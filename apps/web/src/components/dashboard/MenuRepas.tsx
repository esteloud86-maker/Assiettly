...
export function MenuRepas({ mealId }: { mealId: string }) {
  ...
  return (
    <div className="relative">
      ...
      {ouvert ? (
        <div className="absolute right-0 top-12 z-10 w-44 rounded-xl bg-creme-50 py-1 shadow-lg">
          <form action={supprimerRepasEtRevenir.bind(null, mealId)}>
            <button type="submit" className="w-full px-4 py-3 text-left text-sm text-corail-600 hover:bg-creme-200">
              Supprimer ce repas
            </button>
          </form>
        </div>
